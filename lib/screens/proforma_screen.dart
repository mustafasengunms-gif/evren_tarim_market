...
class _ProformaHazirlamaFormuState extends State<ProformaHazirlamaFormu> {
  ...
  Future<void> _stoklariGetir() async {
    ...
    try {
      ...
      final List<Map<String, dynamic>> yerelStoklar = await DatabaseHelper.instance.stokListesiGetir();

      setState(() {
        _stoklar = yerelStoklar.map((s) {
          // 1. İsim Hazırlığı: Önce veritabanındaki 'urun' sütununa bak,
          // yoksa marka-model birleştir.
          String urunSutun = s['urun']?.toString() ?? "";
          String marka = s['marka']?.toString() ?? "";
          String model = s['model']?.toString() ?? "";
          String altModel = (s['alt_model'] ?? s['altmodel'] ?? "").toString();

          String tamAd = urunSutun.isNotEmpty
              ? urunSutun
              : "$marka $model $altModel".trim();

          // 2. Map oluşturma: UI'ın beklediği anahtar kelimelerle (ad, fiyat, miktar)
          return {
            'id': s['id'],
            'ad': tamAd.isEmpty ? "İSİMSİZ ÜRÜN" : tamAd.toUpperCase(),

            // Fiyat Garantisi: SQLite 'fiyat' veya 'alis_fiyatı' olarak ne varsa alıyoruz
            'fiyat': double.tryParse((s['fiyat'] ?? s['alis_fiyatı'] ?? '0').toString()) ?? 0.0,

            'marka': marka,
            'model': model,
            'alt_model': altModel,

            // Miktar Garantisi: SQLite 'adet' sütununa bakıyoruz
            'miktar': double.tryParse((s['adet'] ?? '0').toString()) ?? 0.0,

            // Ek bilgiler (Lazım olursa)
            'cari_kod': s['cari_kod'],
            'firma_unvani': s['firma_unvani'] ?? "Bilinmiyor",
      ...
      print("✅ Stoklar yüklendi: ${_stoklar.length} adet ürün listelendi.");

    } catch (e) {
      debugPrint("❌ _stoklariGetir HATA: $e");
